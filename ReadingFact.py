...
configfilename = os.path.abspath(os.path.dirname(sys.argv[0])) + '\ReadingFact.ini'
...
if str_date:
    ...
    filter = "&filter={{start_date ge %s}} or {{stop_date ge %s}}" % (datefrom.strftime('%Y-%m-%dT%H:%M:%S'), date_to.strftime('%Y-%m-%dT%H:%M:%S'))

path = os.path.abspath(os.path.dirname(sys.argv[0])) + '\\' + folder
    ...
    with open(path+ '\entity_route_sheet_transaction.csv', 'w') as fout:
        ...
        if j['meta']['count'] != 0:
            ...
            for item in j['entity_route_sheet_transaction']:
                ...
                fout.write(line)
                filter_for_entity_route_sheet_operation = filter_for_entity_route_sheet_operation + \
                                                          str(item['entity_route_sheet_operation_id']) + ','
            filter_for_entity_route_sheet_operation = filter_for_entity_route_sheet_operation[:len(filter_for_entity_route_sheet_operation)-1] + ']'

if is_entity_route_sheet_operation:
    filter_str = 'filter={{id in %s}}' % (filter_for_entity_route_sheet_operation)
...
if is_entity_route_sheet:
    filter_by_entity = 'filter={{id in %s}}' % (filter_for_entity_route_sheet)
...
if is_entity_batch:
    ...
    responce = req.get(str_request, cookies=c, headers=h)
    j = json.loads(responce.text)
    with open(path+'\entity_batch.csv', 'w') as fout:
